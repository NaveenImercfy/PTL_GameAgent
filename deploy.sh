...
PROJECT_ID="${GOOGLE_CLOUD_PROJECT:-aitrack-29a9e}"
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars="GOOGLE_API_KEY=$GOOGLE_API_KEY,QUESTION_API_STD=$QUESTION_API_STD,QUESTIONS_SOURCE_API_URL=$QUESTIONS_SOURCE_API_URL,QUESTIONS_SOURCE_API_METHOD=$QUESTIONS_SOURCE_API_METHOD,USE_FIRESTORE=true,GOOGLE_CLOUD_PROJECT=$PROJECT_ID" \
  --memory 1Gi \
  --cpu 1 \
  --timeout 60 \
  --concurrency 80 \
